...
@jit(nopython=True)
def process_halos_chunk_from_bins(
    x, y, z, vx, vy, vz, logM, halo_ids, random_seeds,
    M_min, M_max, Ncen_mean, Nsat_mean,
    K, omega_M, Lbox, zsnap
):
    n_halos = len(x)
    initial_estimate = n_halos * 50
    max_galaxies = int(initial_estimate)
    ...
    for i in range(n_halos):
        ...
        if len(idx) == 0:
            ...
            Ncen = int(np.random.rand() < mean_Ncen)
        ...
        if Ncen == 1:
            ...
            galaxies[galaxy_count, 6] = M
            galaxies[galaxy_count, 7] = Nsat
            galaxies[galaxy_count, 8:14] = 0.0
            galaxies[galaxy_count, 14] = halo_id
            galaxies[galaxy_count, 15] = 1.0
            galaxy_count += 1
            ...
            for chunk_data in io.read_halo_data_chunked(params.infile, params.ftype, chunk_size):
                ...
                if params.analytical_shape:
                    galaxies = process_halos_chunk_analytical(
                        ...
                        analytical_shape=params.analytical_shape, hod_shape_file=params.hod_shape_file
                    )
                else:
                    M_min, M_max, Ncen_mean, Nsat_mean = io.read_occupation_from_h5(params.hod_shape_file)
                    galaxies = process_halos_chunk_from_bins(
                                x, y, z, vx, vy, vz, logM, halo_ids, random_seeds,
                                K = params.K,
                                omega_M = params.omega_M, Lbox = params.Lbox, zsnap = params.zsnap,
                                M_min=M_min, M_max=M_max, Ncen_mean=Ncen_mean, Nsat_mean=Nsat_mean)
...
def run_hod_model(params,verbose=False):
    ...
    if not params.analytical_shape:
        from src.hod_io import read_occupation_from_h5
        M_min, M_max, Ncen_mean, Nsat_mean = read_occupation_from_h5(params.hod_shape_file)
    else:
